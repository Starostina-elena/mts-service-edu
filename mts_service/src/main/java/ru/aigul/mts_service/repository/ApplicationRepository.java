...
public interface ApplicationRepository extends JpaRepository<Application, Long> {
    ...
    @Query("SELECT a FROM Application a JOIN FETCH a.user JOIN FETCH a.tariff WHERE a.status = :status ORDER BY a.id ASC")
    ...
    @Query("SELECT a FROM Application a JOIN FETCH a.user JOIN FETCH a.tariff WHERE a.user.id = :userId ORDER BY a.id ASC")
    List<Application> findAllByUser(@Param("userId") Long userId, Pageable pageable);

    @Query("SELECT a FROM Application a JOIN FETCH a.user JOIN FETCH a.tariff WHERE a.user.id = :userId AND a.id > :after ORDER BY a.id ASC")
    List<Application> findAllByUserAfter(@Param("userId") Long userId, @Param("after") Long after, Pageable pageable);

    @Query("SELECT a FROM Application a JOIN FETCH a.user JOIN FETCH a.tariff WHERE a.user.id = :userId AND a.status = :status ORDER BY a.id ASC")
    List<Application> findAllByUserAndStatus(@Param("userId") Long userId, @Param("status") ApplicationStatus status, Pageable pageable);

    @Query("SELECT a FROM Application a JOIN FETCH a.user JOIN FETCH a.tariff WHERE a.user.id = :userId AND a.status = :status AND a.id > :after ORDER BY a.id ASC")
